...
public class Shopping_Mall {
	...
	public static void main(String args[]) {
		Intro();
		if (s1.equalsIgnoreCase("NEW")) List();
		else if (SignIn()) List();
		else SignIn();
		Calculate();
		Bill();
	...
	public static boolean SignIn() {
		...
		tpass = sc.next();
		sc.nextLine();
		if (tname.equals(properties.getProperty("Name")) && tpass.equals(properties.getProperty("Password"))) return true;
		else return false;
	}
	...
	public static void Calculate() {
		...
		if (s1 == "NEW") total2 = total1 * (100 - dis1) / 100;
		else total2 = total1 * (100 - dis2) / 100;
	}
